...
class AiPipeline():
    ...
    def detector_selector(self, image):
        ...
        cropped_image = ImageProcessor.crop_original_image(box_coordinates[0], image)

        # image_288 = ImageProcessor.preprocess(cropped_image, constants.TARGET_SIZE)

        padded_img = ImageProcessor.resize_and_pad(cropped_image, constants.TARGET_SIZE)

        product_image = ImageProcessor.preprocess_for_embedding_model(padded_img)

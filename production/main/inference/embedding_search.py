...
class EmbeddingSearch:
    def __init__(self):
        ...
        self.neighburs_to_look = 50  # Increased to get more candidates for unique filtering

        if os.path.exists(os.path.join(constants.INDEX_DIR, "scann_config.pb")):
            self.loaded_searcher = scann.scann_ops_pybind.load_searcher(constants.INDEX_DIR)
    ...
    def load_labels(self):
        ...
        LoggerClass.info(f"Loaded {len(labels)} labels from {constants.H5_PATH}")
        return labels
    ...
    def load_embeddings(self):
        ...
        LoggerClass.info(f"Loaded {len(embeddings)} embeddings and labels from {constants.H5_PATH}")
        return embeddings
    ...
    def index_construction(self):
        ...
        searcher = scann.scann_ops_pybind.builder(
            ...
            training_sample_size=len(embeddings) # n of vectors used to train the internal quantization
        ).score_ah(
            dimensions_per_block=2, # n of splits for processing a single vector (trade of between acc and speed)
            anisotropic_quantization_threshold=0.2
        ).reorder(30).build() # n of neighbors to precisely order
    ...
    def process_model_outputs(self, embeddings) -> list:
        ...
        for i, idx in enumerate(neighbors):
            if idx < len(self.labels):
                ...
                LoggerClass.error(f"Index {idx} out of range for labels with length {len(self.labels)}")
        
        LoggerClass.info(f"Found {len(top_barcodes)} unique barcodes from {len(neighbors)} neighbors")
        return top_barcodes
